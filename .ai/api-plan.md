...
- **Accounts**
  - *Database Tables*: `auth.users`, `app.profiles`
...
- **Generations**
  - *Database Table*: `app.ai_generation_logs`
...
- **Generation Error Logs**
  - *Database Table*: `app.ai_generation_error_logs`
...
- **Study Sessions**
  - *Virtual Resource*: generated by repetition engine fed from `app.flashcards`. Session state may be cached in-memory/temporary store; future persistence can live in dedicated table.

- **AI Metrics**
  - *Virtual Resource*: Aggregations over `app.ai_generation_logs` (per day, per model, etc.) used for KPI dashboards.
